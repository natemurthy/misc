...
`../tests/test_1988_td.py` runs the shared interface checks and confirms the split above: while learning, the agent never calls `CartPoleEnv.dynamics`; only the frozen policy's one-step lookahead does.
